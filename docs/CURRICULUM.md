...
**LUNA работает в учебном режиме Codex.** Правила в [`AGENTS.md`](../AGENTS.md) запрещают ассистенту менять `artifacts/` и читать `solution/` во время самостоятельной попытки: тесты он может запускать, но код пишете вы.
